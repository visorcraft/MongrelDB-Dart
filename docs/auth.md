...
The `mongreldb-server` daemon has two auth modes. The Dart client supports
both, and sends credentials only in the `Authorization` header.

## Bearer token (--auth-token mode)

Pass a `token` when constructing the client. Every request carries an
...
A 401 or 403 from the daemon raises an [AuthException]. Catch it to react:

```dart
try {
  await db.put('orders', {1: 1});
