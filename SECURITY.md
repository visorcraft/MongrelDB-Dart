...
This document describes the security properties of the MongrelDB Dart client
and how to report vulnerabilities.

## Overview

The MongrelDB Dart client is a pub package that talks to `mongreldb-server`
...
- Server and network errors are mapped to the typed exception hierarchy
  ...
  `ConnectionException`, `QueryException`), not leaked as generic
  exceptions.
...
The MongrelDB Dart client has no runtime dependencies beyond the Dart SDK.
Development dependencies (`test`, `lints`) are MIT or BSD-licensed. Report
...
- the MongrelDB Dart client version, Dart version, and OS,
